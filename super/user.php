...
<head>
    <meta charset="utf-8" />
    <title>Super || System Users</title>
                    ...
                    <table id="tblcoa" class="table-bordered table-head-fixed table-striped table-responsive text-nowrap">
                        ...
                        foreach ($response['data'] as $row) { $id = $dmo->safeData($row['id']); ?>
                            <tr>
                                ...
                                <td contentEditable=true onblur="edit('user','username','<?= $id ?>',this)"><?= $dmo->safeData($row['username']) ?></td>
                                <td contentEditable=true onblur="edit('user','displayname','<?= $id ?>',this)"><?= $dmo->safeData($row['displayname']) ?></td>
                                ...
                                <td contentEditable=true onblur="edit('user','email','<?= $id ?>',this)"><?= $dmo->safeData($row['email']) ?></td>
                                <td contentEditable=true onblur="edit('user','contact','<?= $id ?>',this)"><?= $dmo->safeData($row['contact']) ?></td>
                                ...
                                <td><a href="#" class="btn btn-xs btn-<?=$row['status']? "danger" : "success";?>" ><i class="fas fa-<?=$row['status']? "lock" : "lock-open"?>"></i>&nbsp;<?= $row['status']? "Deactivate" : "Activate" ?></a></td>
                            </tr>
                        <?php $count++; } }?>
                        </tbody>
                    </table>

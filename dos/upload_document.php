...
<head>
    <meta charset="utf-8" />
    <title>D.O.S || Upload Documents</title>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card-box">  
                        <table id="tbldocumentupload" class="table-bordered table-head-fixed table-striped table-responsive text-nowrap">
                            ...
                            <?php
                            if($dmo->getDocumentUploads(['school_code'=>$user['school_code']])['status']){
                            $response = $dmo->getDocumentUploads(['school_code'=>$user['school_code']]); $count=1;
                            foreach ($response['data'] as $row) { $id = $dmo->safeData($row['id']); ?>
                                <tr>
                                    <td><?= $count ?></td>
